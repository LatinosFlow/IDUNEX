# SOURCE_TRACE_TO_RUNTIME_CONTRACT

## Propósito
Hace exigible que cada fuente llegue a ejecución: prompt, QA, fallback, sidecar y manifest.

## Autoridad
Autoridad activa: `IDUNEX/05_SOURCE_TO_RUNTIME/02_MAPS/SOURCE_TO_RUNTIME_MASTER_MAP.json`. Este archivo es contrato/bridge operativo, no resumen. Si existe un documento largo con el mismo dominio, este bridge define punto de entrada, ruta de carga, criterios ejecutables y bloqueo QA. No compite con PROJECT_CORE; PROJECT_CORE es autoridad superior para proyectos exportados.

## Alcance
SRC_001-SRC_049, source cards, source inventory, runtime domains, maps, Profile360, Project Core y agentes.. Aplica a motor base `IDUNEX_MOTOR_VERSION = v1.0.0`, Project Factory, Agent Factory, CHATGPT, COPILOT, Profile360, manifests, sidecars, validators, hash lineage y exportación productiva controlada. No autoriza PROJECT_INSTANCE_GO ni OUTPUT_GO sin QA propio.
...
## Load order
1) SOURCE_INVENTORY_MASTER. 2) Source Cards. 3) SOURCE_TO_RUNTIME_MASTER_MAP. 4) Profile360. 5) Agent packs. 6) QA.
...
- `IDUNEX/05_SOURCE_TO_RUNTIME/01_INVENTORY/SOURCE_INVENTORY_MASTER.json`
- `IDUNEX/05_SOURCE_TO_RUNTIME/02_MAPS/SOURCE_TO_RUNTIME_MASTER_MAP.json`
- `IDUNEX/05_SOURCE_TO_RUNTIME/03_SOURCE_CARDS/`
...
- `VALIDATE_SOURCE_MAP_NO_NULL_RUNTIME_KEYS`
- `VALIDATE_SOURCE_MAP_NO_EMPTY_RUNTIME_KEYS`
- `VALIDATE_SOURCE_MAP_REQUIRED_KEYS_STRICT`
- `VALIDATE_SOURCE_TRACE_TO_AGENT_PACKS`
- `VALIDATE_SRC049_ALIAS_EXCLUSIVE`
- `VALIDATE_CANONICAL_CONTRACTS_NOT_SUMMARY_ONLY`

## Fail codes
- `FAIL_SOURCE_TRACE_NULL`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_SOURCE_TRACE_EMPTY`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_SRC049_DOUBLE_COUNT`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_SOURCE_RUNTIME_NOT_REACHED`: bloquea entrega hasta fix, evidencia y re-test.
...
## Ejemplos PASS/FAIL
- PASS: SRC_049 tiene alias ENV_PHYSICS_FULL10 exclusivo y dos archivos físicos sin doble conteo.
- FAIL: fuente citada solo en documentación y ausente en prompt/QA/fallback/sidecar.
...
## Relación operativa
La investigación viaja a PROJECT_CORE, CHATGPT, COPILOT, Profile360, QA, sidecars y vendor guides.
...
Canonical chain literal: Research Source -> Source Card -> Profile360 Field -> Project Core Rule -> ChatGPT Agent Rule -> Copilot Agent Rule -> Model Profile Field -> Prompt / Output / QA / Sidecar.


## SRC_049 family / no double count evidence

- canonical_source_id: `SRC_049_ENV_PHYSICS_FULL10`
- source_alias: `ENV_PHYSICS_FULL10`
- physical_family_files: `SRC_049_ENV_PHYSICS_FULL10_SOURCE_CARD.md`, `SRC_049_SCENE_ENVIRONMENT_SPATIAL_PHYSICS_ENGINE_FULL10.md`
- canonical_source_count: `49`
- source_count_policy: `ONE_CANONICAL_SOURCE_ID_WITH_TWO_COMPLEMENTARY_PHYSICAL_FILES`
- double_count_status: `NO_DOUBLE_COUNT_PASS`

## QA_GAUNTLET canonical route evidence

- `IDUNEX/22_QA_GAUNTLET_CANONICAL_BRIDGE`: registry / canonical bridge / gate.
- `IDUNEX/13_QA_GAUNTLET`: operational library / legacy-compatible detail.
- authority_status: `NO_CONTRADICTORY_DOUBLE_AUTHORITY`
...
## PHASE 1/4 — SRC_049 canonical lookup rule
Canonical source lookup MUST resolve SRC_049 by source_id_canonical, source_alias and runtime_module, not only by source_id.
canonical_lookup_keys = source_id, source_id_original, source_id_canonical, source_alias, runtime_module
validator = VALIDATE_SRC049_CANONICAL_LOOKUP_ACCEPTS_SOURCE_ID_AND_SOURCE_ID_CANONICAL
updated_at = NEUTRALIZED_ACTIVE_SCOPE
