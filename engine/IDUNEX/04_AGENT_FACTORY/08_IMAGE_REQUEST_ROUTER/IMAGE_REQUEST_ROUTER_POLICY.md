...
`IMAGE_REQUEST_ROUTER=ON` for any colloquial or explicit visual request. Activators include genera, crea, foto, imagen, retrato, rostro, cuerpo completo, casual, deportivo, vestido, fondo blanco, macro, primer plano and model-name + scene/wardrobe requests. A new image request must route to the image tool. Python/manual composite is blocked for visual generation; Python is allowed only after an asset exists for QA/hash/sidecar work.
