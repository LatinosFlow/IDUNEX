...
# AGENT_FACTORY_FULL_CONTRACT

## Active authority clarification — 1ADD3F8D
- active_authority_target: `IDUNEX/04_AGENT_FACTORY/05_AGENT_FACTORY_FULL_CONTRACT.md`
...
## Propósito
Bridge de autoridad completa de Agent Factory para generación, auditoría y entrega de agentes IDUNEX.

## Autoridad
Autoridad activa: `IDUNEX/04_AGENT_FACTORY/05_AGENT_FACTORY_FULL_CONTRACT.md`. Este archivo es contrato/bridge operativo, no resumen. Si existe un documento largo con el mismo dominio, este bridge define punto de entrada, ruta de carga, criterios ejecutables y bloqueo QA. No compite con PROJECT_CORE; PROJECT_CORE es autoridad superior para proyectos exportados.

## Alcance
Agentes multimodales, lectura obligatoria, control de identidad, prompts, diálogos, música, voz, video, QA y sidecars.. Aplica a motor base `IDUNEX_MOTOR_VERSION = v1.0.0`, Project Factory, Agent Factory, CHATGPT, COPILOT, Profile360, manifests, sidecars, validators, hash lineage y exportación productiva controlada. No autoriza PROJECT_INSTANCE_GO ni OUTPUT_GO sin QA propio.
...
## Load order
1) Control Center. 2) PROJECT_CORE. 3) Agent Factory full. 4) Multimodal Contracts. 5) QA Gauntlet. 6) Export Packer.
...
- `IDUNEX/04_AGENT_FACTORY/05_AGENT_FACTORY_FULL_CONTRACT.md`
- `IDUNEX/06_MULTIMODAL_CONTRACTS/`
- `IDUNEX/22_QA_GAUNTLET_CANONICAL_BRIDGE/VALIDATORS_PRODUCTIVE_BASE_ENGINE.md`
...
## Fail codes
- `FAIL_AGENT_MEMORY_FREE`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_MODEL_DIALOGUE_DRIFT`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_REFERENCE_FACE_COPY`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_AUDIO_SIDECAR_MISSING`: bloquea entrega hasta fix, evidencia y re-test.
...
## Ejemplos PASS/FAIL
- PASS: el agente conversa como modelo con límites, genera prompt, QA, fallback y sidecar.
- FAIL: el agente inventa canon bloqueado o mezcla identidades entre modelos.
...
## Relación operativa
Agent Factory consume Profile360 y Model Causality; Scene Physics bloquea outputs con contacto, escala o luz incoherente.
...
## P0 alias/autoload addition

Agent Factory configs must include MODEL_ALIAS_RESOLVER_AND_AUTOLOAD, blocking codes, read-real-files-first, 10 core + 1 full MODEL_RUNTIME_PROFILE_FULL per model and max 20 knowledge files.
