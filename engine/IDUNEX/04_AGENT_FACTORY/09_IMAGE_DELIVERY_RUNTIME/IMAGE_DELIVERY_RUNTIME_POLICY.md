...
Never leave the user with an empty answer. If the image tool does not return a visible asset, return a diagnostic with `IMAGE_TOOL_NO_ASSET_RETURNED` or `IMAGE_REQUEST_ROUTED_BUT_NO_VISIBLE_OUTPUT`. Deliver `PREVIEW_RENDER` when the platform renders an image, but do not certify it as `OUTPUT_REAL_10_10` without full evidence.
