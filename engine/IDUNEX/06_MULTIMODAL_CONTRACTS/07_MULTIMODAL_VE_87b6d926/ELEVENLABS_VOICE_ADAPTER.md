# ELEVENLABS VOICE ADAPTER — contrato operativo completo

## INPUT CONTRACT
El adapter `elevenlabs_voice_adapter` recibe únicamente campos Perfil360 con `adapter_targets` que incluyan `elevenlabs_voice_adapter`, source_ids reales, locks activos, estado permitido y sidecar_trace=true. No acepta campos sin runtime_domain granular ni campos derivados no justificados.

## OUTPUT CONTRACT
El output debe producir una especificación usable por el canal voz ElevenLabs, con secciones verificables, source trace, field trace, QA before output, fail codes, fallbacks y sidecar requirements. Ningún resultado puede omitir `adapter_used`, `source_ids_used`, `field_ids_used` ni producción state.

## REQUIRED PROFILE360 FIELDS
Debe cargar campos de identidad, edad adulta, realismo humano, continuidad, legal/governance y todos los campos específicos del adapter. Para voz ElevenLabs, las secciones obligatorias son: voice_age_signal, voice_identity_lock, timbre_signature, pitch_range, resonance_place, breath_pattern, speaking_rate, prosody_curve, pause_signature, accent_region, peruvian_spanish_level, latam_neutrality_rule, sociolect_level, emotional_range, speaker_drift_blocker, celebrity_imitation_blocker.
...
## PARAMETER TABLE
Parámetros mínimos: voice_age, timbre, pitch_range, speaking_rate, breathiness, nasality, resonance, vocal_weight, accent_region, sociolect_level, noise_floor, mic_distance. Cada parámetro debe tener valor declarado, rango permitido, rango prohibido, método QA, fail_code y fallback.
...
El sidecar debe incluir output_id, project_id, model_id si aplica, engine_version, source_ids_used, field_ids_used, adapter_used=elevenlabs_voice_adapter, vendor_params, locks_active, qa_status, fail_codes, fallbacks_applied, hash_input, hash_output y production_state.
...
voice_age_signal, timbre_signature, pitch_range, resonance_place, breath_pattern, speaking_rate, prosody_curve, pause_signature, phoneme_stress, accent_region, peruvian_spanish_level, latam_neutrality_rule, sociolect_level, emotional_range, speaker_drift_blocker, celebrity_imitation_blocker, mic_distance, noise_floor.
