...
## Propósito
Bridge canónico del contrato largo de Project Factory; evita que el índice corto sea tomado como resumen suficiente.
...
## Alcance
Carga, expansión no-loss, estados FACTORY_DEFINED/USER_APPROVED_LOCKED y reglas de exportación de proyectos.. Aplica a motor base `IDUNEX_MOTOR_VERSION = v1.0.0`, Project Factory, Agent Factory, CHATGPT, COPILOT, Profile360, manifests, sidecars, validators, hash lineage y exportación productiva controlada. No autoriza PROJECT_INSTANCE_GO ni OUTPUT_GO sin QA propio.
...
## Load order
1) Leer este bridge. 2) Cargar `05_PROJECT_FACTORY_FULL_CONTRACT.md`. 3) Cargar plantillas de proyecto. 4) Validar QA y manifests.
...
- `IDUNEX/03_PROJECT_FACTORY/01_TEMPLATES/PROJECT_MANIFEST_TEMPLATE.json`
- `IDUNEX/03_PROJECT_FACTORY/01_TEMPLATES/IDUNEX_PROJECT_TEMPLATE_v1.0.0/PROJECT_PACKAGE_MANIFEST.json`
...
## Fail codes
- `FAIL_FACTORY_FIELD_EMPTY`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_USER_LOCK_MUTATED`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_PROJECT_CORE_NOT_AUTHORITY`: bloquea entrega hasta fix, evidencia y re-test.
- `FAIL_SUMMARY_CONTRACT`: bloquea entrega hasta fix, evidencia y re-test.
...
## Ejemplos PASS/FAIL
- PASS: campo faltante queda FACTORY_DEFINED con source_trace, QA y campo editable.
- FAIL: se pide dato menor al usuario y se deja campo genérico sin causalidad.
...
## Relación operativa
Project Factory entrega estructura; Agent Factory adapta. Copilot mantiene paridad documental y no se degrada frente a ChatGPT.
