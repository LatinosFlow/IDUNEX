# Vendor adapters, sidecar, watermark, legal, no-loss y governance
...
## LEGACY_NON_AUTHORITY Runtime Evidence and Agent Fit
Agent runtime policy: one ChatGPT agent and one Copilot agent per project, exactly 10 core files + 1 MODEL_RUNTIME_PROFILE_FULL per model, max 20 files, max 10 models, no split agents, no destructive summary. PROJECT_RUNTIME_COVERAGE_MAP is mandatory. OUTPUT_REAL_10_10 requires real asset, sidecar, prompt_hash, output_hash, QA, reviewer and lineage.
