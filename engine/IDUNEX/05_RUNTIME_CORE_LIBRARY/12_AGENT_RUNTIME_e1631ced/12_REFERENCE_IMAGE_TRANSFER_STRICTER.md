# REFERENCE_IMAGE_TRANSFER_STRICTER

## Purpose
Strengthens reference image transfer.

## Operational rules
- Allowed: pose, framing, light, atmosphere, mood, compatible clothing, composition, environment
- Forbidden: real third-party facial identity, minor appearance, non-canonical body when model locked, protected logo/brand, traits contradicting Profile360

## Required evidence
- files_checked
- rules_checked
- expected_value
- actual_value
- result
- timestamp

## Runtime connection
This policy must be referenced by Project Factory, Agent Factory, ChatGPT config, Copilot config, prompt packs, QA, sidecar and release validator.

## Fail codes
- RUNTIME-SOURCE-MISSING
- SUMMARY-LOSS
- CANON-INVENTED
- OUTPUT-GATE-BYPASS
- WATERMARK-POLICY-MISSING
- VENDOR-HANDOFF-INCOMPLETE

## Fallback fixes
- Re-load required files.
- Convert destructive summary into canonical compilation.
- Mark new values as FACTORY_DEFINED_PROPOSED.
- Return output status to DRAFT or QA_PENDING.
- Generate missing sidecar/hash/lineage evidence.
