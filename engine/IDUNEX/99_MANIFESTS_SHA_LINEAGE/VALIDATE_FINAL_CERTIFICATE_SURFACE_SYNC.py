...
DUPLICATE_SELF_EXCLUDED={
 '14_HISTORICAL_NON_AUTHORITY/AUD_008_ACTIVE_HISTORY/99_MANIFESTS_SHA_LINEAGE/ENGINE_EXACT_DUPLICATE_ALLOWLIST.json','14_HISTORICAL_NON_AUTHORITY/AUD_008_ACTIVE_HISTORY/99_MANIFESTS_SHA_LINEAGE/EXACT_DUPLICATE_ALLOWLIST.json',
...
def engine_mode(root: Path):
    _vf=_active_version_v1_0_0_guard(root)
    if _vf:
        return out("FAIL", fail_codes=sorted({f.get('code','FAIL_ACTIVE_VERSION_RENORMALIZATION_GUARD') for f in _vf}), failures=_vf, validators_fail=len(_vf))
    ...
    if not _as_ok:
        return out("FAIL", fail_codes=sorted({f.get('code','FAIL_ACTIVE_SURFACE_SCOPE_SYNC') for f in _as_failures}), failures=_as_failures, validators_fail=len(_as_failures))
    ...
    return out("PASS", scope=CURRENT_SCOPE, previous_scope_compatibility=PREVIOUS_SCOPE_COMPATIBILITY, legacy_scope_compatibility=LEGACY_SCOPE_COMPATIBILITY, engine_contract="PASS", correction_scope_gate="PASS")
